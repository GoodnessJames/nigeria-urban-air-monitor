...
# Pollutants displayed by the dashboard.
POLLUTANTS = {
    "PM2.5": "pm2_5",
    "PM10": "pm10",
    "NO₂": "nitrogen_dioxide",
...
# Open-Meteo provides pollutant-specific US AQI for these pollutants.
# Dust does not have a pollutant-specific US AQI field in Open-Meteo.
POLLUTANT_US_AQI = {
    "PM2.5": "us_aqi_pm2_5",
    "PM10": "us_aqi_pm10",
...
def initialize_database() -> None:
    ...
    # add the new pollutant-specific AQI columns.
    # --------------------------------------------------------
    ...
    for column, data_type in new_aqi_columns.items():
        con.execute(
            f"""
...
def parse_hourly_response(
    ...
    """Convert an Open-Meteo hourly response to our schema."""
    ...
    if cutoff_local is not None:
        ...
        cutoff_naive = cutoff_local.replace(
            tzinfo=None
        )
    ...
    with DB_LOCK:
        ...
        if (
            ...
            return {
                ...
                "rows_inserted": 0,
                "message": (
                    "Recent ingestion already completed."
                ),
            }
...
def backfill_history(
    ...
    """
    Create an initial historical dataset.
    """
    ...
    if (
        ...
        return {
            ...
            "rows_inserted": 0,
            "message":
                "Historical backfill already exists.",
        }
...
def query_baseline() -> pd.DataFrame:
    ...
    """Current PM2.5 versus each city's seven-day average."""
...
def build_status_cards(
    ...
    """Return current city cards ordered worst → best."""
...
def live_dashboard():
    ...
    st.markdown(
        ...
        'Select a city and pollutant to inspect '
        'the current concentration, pollutant-specific '
        'US AQI and category.'
        '</div>',
        unsafe_allow_html=True,
    )
    ...
    for column, (_, row) in zip(
        ...
        with column:
            ...
            # ----------------------------------------------
            # Get pollutant-specific AQI directly from
            # Open-Meteo.
            # ----------------------------------------------
            ...
            if pd.notna(
                ...
                aqi_text = (
                    f"US AQI {float(pollutant_aqi):.0f}"
                )
    ...
    if selected_pollutant == "Dust":

        st.caption(
            "Dust concentration is available from "
            "Open-Meteo, but Open-Meteo does not provide "
            "a pollutant-specific US AQI value for Dust."
        )
    ...
    st.caption(
        ...
        "Hover over a cell for the exact current value."
    )
    ...
    st.caption(
        ...
        "a seasonal-naive 24-hour baseline and uses "
        "the better model. The shaded interval represents "
    ...
    st.markdown(
        ...
        <div class="insight">
            ...
            currently has the highest PM2.5
            concentration among the three monitored
            cities at
    ...
    st.markdown(
        ...
        <b>Data source:</b>
        Open-Meteo Air Quality API using
        CAMS-based modelled air-quality data.

        Values represent model output rather than
        street-level sensor measurements.

        Pollutant-specific US AQI values are supplied
        directly by Open-Meteo.

        AQI categories are presented using the
        standard US AQI bands.
